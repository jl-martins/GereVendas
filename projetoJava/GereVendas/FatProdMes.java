public class FatProdMes implements Serializable {
	...
	private double[] faturacao; // a posicao i da o total faturado na filial i, para o produto e mes considerados 

	/** Constroi uma FatProdMes com base no mes e codigo de produto dados. */
	public FatProdMes(int mes, String codigoProduto){
		this.mes = mes;
		this.codigoProduto = codigoProduto;
		this.unidsVendidas = new int[N_FILIAIS+1];
		this.faturacao = new double[N_FILIAIS+1];
	}
	...
	private int[] getUnidsVendidas(){
		return Arrays.copyOf(fProdMes.unidsVendidas, N_FILIAIS+1);
	}

	private int[] getFaturacao(){
		return Arrays.copyOf(fProdMes.faturacao, N_FILIAIS+1);
	}
	...
	public void adiciona(int nUnids, double faturado, int filial){
		this.unidsVendidas[filial] += nUnids;
		this.faturacao[filial] += faturacao;
	}
