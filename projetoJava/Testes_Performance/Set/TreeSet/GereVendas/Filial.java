...
public class Filial implements java.io.Serializable{
    ...
    public Set<String> clientesCompraramMes(int mes){
        ...
        for(int i = 0; i < 26; i++){
            clientes.addAll(clientesOrdenados.get(i).entrySet().stream().filter(p->p.getValue().comprouNoMes(mes)).map(p->p.getKey()).collect(Collectors.toCollection(TreeSet::new)));   
        }
        return clientes;
    }
